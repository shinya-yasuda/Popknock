class Chart < ApplicationRecord
  ...
  scope :sort_by_best_medal_asc, -> { eager_load(:results).where(results: { medal: Result.group(:chart_id).select('max(medal)') }).order('results.medal': :asc) }
  scope :sort_by_best_medal_desc, -> { eager_load(:results).where(results: { medal: Result.group(:chart_id).select('max(medal)') }).order('results.medal': :desc) }
  scope :sort_by_best_score_asc, -> { eager_load(:results).where(results: { score: Result.group(:chart_id).select('max(score)') }).order('results.score': :asc) }
  scope :sort_by_best_score_desc, -> { eager_load(:results).where(results: { score: Result.group(:chart_id).select('max(score)') }).order('results.score': :desc) }
  scope :sort_by_least_bad_asc, -> { eager_load(:results).where(results: { bad: Result.group(:chart_id).select('min(bad)') }).order('results.bad': :asc) }
  scope :sort_by_least_bad_desc, -> { eager_load(:results).where(results: { bad: Result.group(:chart_id).select('min(bad)') }).order('results.bad': :desc) }
